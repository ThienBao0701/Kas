...
export function CopyField({ label, value, copyValue, mono = false }: CopyFieldProps) {
  ...
  return (
    <div className="flex items-start justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50/60 px-3 py-2.5">
      <div className="min-w-0">
        <p className="text-xs font-medium uppercase tracking-wide text-slate-400">{label}</p>
        <p className={`mt-0.5 break-words text-sm text-slate-900 ${mono ? 'font-mono' : ''}`}>{display}</p>
      </div>
